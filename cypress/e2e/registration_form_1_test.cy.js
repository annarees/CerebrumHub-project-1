...
describe('This is first test suite, Anne Arendi', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('PassMy')
        cy.get('[name="confirm"]').type('PassMy')
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('[data-testid="phoneNumberTestId"]').type('55566677').clear()
        cy.get('input[name="password"]').type('PassMy')
        cy.get('[name="confirm"]').type('PassMy')
        cy.get('.submit_button').should('be.disabled')
        cy.get('#success_message').should('not.be.visible')
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('[data-testid="phoneNumberTestId"]').type('55566677')
        cy.get('input[name="password"]').type('PassMy').clear()
        cy.get('[name="confirm"]').type('PassMy').clear()
        cy.get('.submit_button').should('be.disabled')
        cy.get('#success_message').should('not.be.visible')
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#username').type('Username')
        cy.get('input[name="password"]').type('PassMy{enter}')
        cy.get('[name="confirm"]').type('PassMy')
